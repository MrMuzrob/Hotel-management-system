...
#         return render(request, 'ownerAdmin/admin_login.html')
#     else:
#         form = CreateCustomer(request.POST or None)
#         if form.is_valid():
#             customer = form.save(commit=False)
#             customer.save()
#             return redirect('customers_list')
#         return render(request, 'ownerAdmin/create_customer.html', {'form': form})


# def Customer_list(request, customer_name):

#     customers = Customers.objects.all()
#     customers = customers.filter(customer_name__Name=customer_name)
#     query = request.GET.get("q", None)
#     if query is not None:
#         customers = customers.filter(status__startswith=query)

#     if request.method == "POST":
#         customer_name = request.POST.get('customer_name')
#         customer_phone = request.POST.get('customer_phone')
#         customers= get_object_or_404(Customers, Name=customer_name)
#         customers.save()
#     context = {
#         'customer_name':customer_name,
#         'customer_phone':customer_phone,
#      }
#     return render(request, 'ownerAdmin/customers_list.html', context, {'customer_name':customer_name})



# def delete_Customer(request, customer_first_name):
#     if not request.user.is_authenticated & request.user.is_superuser:
#         return render(request, 'ownerAdmin/admin_login.html')
#     else:
#         if request.method == 'POST':
#             if 'delete' in request.POST:
#                 customer = get_object_or_404(Customers, Name=customer_first_name)
#                 customer.delete()
#                 return redirect('customers_list')
#             elif 'cancel' in request.POST:
#                 return redirect('customers_list')
#         return render(request, 'ownerAdmin/delete_customer.html', {'customer_first_name': customer_first_name})
